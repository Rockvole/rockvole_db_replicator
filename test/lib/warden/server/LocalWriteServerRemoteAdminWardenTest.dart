...
// Based on AversionWardenTest
class LocalWriteServerRemoteAdminWardenTest extends AbstractWardenTest {
  // -------------------------------------------- INSERT
  ...
  LocalWriteServerRemoteAdminWardenTest(
      DbTransaction clientDb, DbTransaction serverDb)
      : super(clientDb, serverDb);

  Future<void> run_test() async {
    await super.run_all_tests(WardenType.WRITE_SERVER, WardenType.ADMIN);
  }
  ...
  Future<void> update_server() async {
    await runServerTest(updateTaskStruct.id, WaterState.CLIENT_STORED,
        updateTaskStruct, updateTrStruct, updateWlStruct, null, null);
  }

  Future<void> delete_server() async {
    await runServerTest(deleteTaskStruct.id, WaterState.CLIENT_STORED,
