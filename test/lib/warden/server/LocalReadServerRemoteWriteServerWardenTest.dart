...
class LocalReadServerRemoteWriteServerWardenTest extends AbstractWardenTest {
  // -------------------------------------------- INSERT
  static TestTrDataStruct insertTrStruct = TestTrDataStruct(
      ...
      user_id: 74,
      user_ts: 2828,
      comment: "Read Server Inserted From Write Server to Task",
      crc: null,
  ...
  LocalReadServerRemoteWriteServerWardenTest(
      DbTransaction clientDb, DbTransaction serverDb)
      : super(clientDb, serverDb);

  Future<void> run_test() async {
    await super.run_all_tests(WardenType.READ_SERVER, WardenType.WRITE_SERVER);
  }

  Future<void> insert_server() async {
    await runServerTest(insertTaskStruct.id, WaterState.SERVER_APPROVED, insertTaskStruct,
